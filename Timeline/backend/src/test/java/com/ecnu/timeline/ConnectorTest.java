...
public class ConnectorTest {
    ...
    public void testLoadMore() throws Exception  {
        ...
        SimpleDateFormat isoFormat = mock(SimpleDateFormat.class);
        when(isoFormat.parse(anyString())).thenReturn(time);


        when(commentRepository.findMoreCommentsBefore(any(Date.class), eq(2))).thenReturn(commentList);

        MvcResult result = this.mockMvc.perform(post("/loadMore").content("{\"time\": \"" + time.toString() + "\"}"))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().contentTypeCompatibleWith(MediaType.TEXT_PLAIN))
                .andDo(print())
                .andReturn();
        ...
        verify(commentRepository).findMoreCommentsBefore(any(), anyInt());
    }
